...
const BranchesPage = () => {
    ...
    const load = async (bid: UUID) => {
        ...
        setErr(null);
        try {
            const [list, tps, countries] = await Promise.all([getBranches(bid), getBranchTypes(bid), getCountries()]);
            setItems(list);
            setTypes(tps);
            setCountryMap(new Map(countries.map((c: any) => [c.id, c.name])));
            const uniqCountryIds = Array.from(new Set(list.map((b) => b.countryId).filter(Boolean) as string[]));
    ...
    const columns = 5 + (canWriteBranches ? 1 : 0);
    ...
    return (
        <div className="bg-white rounded-2xl p-6 shadow-sm">
                ...
                <div className="overflow-x-auto max-h-96 overflow-y-auto">
                    <table className="min-w-full text-sm">
                        ...
                        <tr className="text-left text-gray-500 border-b">
                            <th className="py-2 pr-3 whitespace-nowrap">{t("branches.fields.officeNickname")}</th>
                            ...
                            <th className="py-2 pr-3 w-48">{t("branches.col.location", {defaultValue: "Address"})}</th>
                            <th className="py-2 pr-3 w-32">{t("branches.col.pseudoCityCode", {defaultValue: "PCC"})}</th>
                            {canWriteBranches && <th className="py-2 pr-3 w-40 text-right">{t("common.actions")}</th>}
                        </tr>
                        </thead>
                        <tbody>
                        {items.length === 0 && (
                            <tr>
                                <td className="py-6 text-center text-gray-500" colSpan={columns}>
                                    {t("branches.empty")}
                                </td>
                            </tr>
                        )}
                        {items.map((b) => {
                            ...
                            return (
                                <tr key={b.id} className="border-b last:border-b-0">
                                    ...
                                    <td className="py-2 pr-3 w-48">{b.location?.trim() || "—"}</td>
                                    <td className="py-2 pr-3 w-32">
                                        {b.pseudoCityCode?.length ? (
                                            <div className="flex flex-wrap gap-1">
                                                {b.pseudoCityCode.map((code, i) => (
                                                    <span key={`${b.id}-pcc-${i}`}
                                                          className="px-2 py-1 rounded-lg bg-gray-100 text-gray-700"
                                                          title={code}>
                                                        {code}
                                                    </span>
                                                ))}
            ...
            <ConfirmModal
                ...
                text={deleting ? t("branches.delete.text", { name: deleting.name }) : t("branches.delete.textShort")}
