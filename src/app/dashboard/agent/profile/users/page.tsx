...
export default function UsersPage() {
    ...
    const loadAll = async (bid: UUID) => {
        ...
        try {
            ...
            // Загружаем локации параллельно, если они еще не загружены
                ...
                <div className="overflow-x-auto max-h-96">
                    <table className="min-w-full text-sm">
                        ...
                        <tr className="text-left text-gray-500 border-b">
                            <th className="py-2 pr-3 whitespace-nowrap">{t("users.col.firstName")}</th>
                            <th className="py-2 pr-3 whitespace-nowrap">{t("users.col.lastName")}</th>
                            <th className="py-2 pr-3 whitespace-nowrap">{t("users.col.branch")}</th>
                            <th className="py-2 pr-3 whitespace-nowrap">{t("users.col.structure")}</th>
                            <th className="py-2 pr-3 whitespace-nowrap">{t("users.col.role")}</th>
                            <th className="py-2 pr-3 whitespace-nowrap">{t("users.col.email")}</th>
                            <th className="py-2 pr-3 whitespace-nowrap">{t("users.col.phone")}</th>
                            <th className="py-2 pr-3 whitespace-nowrap">{t("users.col.extensionPhone")}</th>
                            {
                                canWriteUsers && (
                                    <th className="py-2 pr-3 w-40 text-right whitespace-nowrap">{t("users.col.actions")}</th>
                        ...
                        {items.length === 0 && (
                            <tr>
                                <td className="py-6 text-center text-gray-500" colSpan={canWriteUsers ? 9 : 8}>
                                    {t("users.empty")}
                                </td>
                            </tr>
                        )}
                        {items.map((u) => (
                            <tr key={u.id} className="border-b last:border-b-0">
                                ...
                                <td className="py-2 pr-3 whitespace-nowrap">
                                    {u.branchId ? branchNameById.get(u.branchId as UUID) || "—" : "—"}
                                </td>
                                <td className="py-2 pr-3 whitespace-nowrap">{u.structureId ? structureNameById.get(u.structureId as UUID) || "—" : "—"}</td>
