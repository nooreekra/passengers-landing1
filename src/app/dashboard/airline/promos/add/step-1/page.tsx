...
export default function AddPromoStep1() {
    ...
    const schema = z
        .object({
            ...
            imageFile: z.instanceof(File, { message: REQ }),
            imageUri: z.string().url().optional(),
        })
        .refine((d) => !!d.imageFile || !!d.imageUri, {
            path: ["imageFile"],
            message: REQ,
        });
        ...
        try {
            ...
            if (existingId) {
                setData({
                    ...
                    imageUri: form.imageUri ?? data.imageUri,
                });
                router.push(`/dashboard/airline/promos/add/step-2?promoId=${existingId}`);
                return;
            }
            ...
            if (businessId) {
                ...
                setData({
                    ...
                    imageUri: promo.imageUri ?? undefined,
                    promoId: promo.id,
                });

                router.push(`/dashboard/airline/promos/add/step-2?promoId=${promo.id}`);
            } else {
                    ...
                    <Dropzone
                        ...
                        onSelect={async (f) => {
                            ...
                            try {
                                ...
                                setData((prev) => ({ ...prev, imageUri: url }));
                            } catch (e) {
                                console.error(e);
                            }
                        }}
